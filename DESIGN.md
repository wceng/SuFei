...
### 4.1 层级反馈
- **触感反馈**：所有可点击项（如诗词卡片、印章按钮）集成轻微的触感反馈（Haptic Feedback），增强物理质感。
- **多维联动**：在“万卷”页面，用户可以同时勾选“宋代”+“苏轼”+“定风波”，实现极精准的定位。

### 4.2 动效规范 (Animation Spec)
遵循“轻量、平滑、具备文学连贯性”的原则。
- **全局转场 (Global Transitions)**：
  - **顶级页面切换**：如在“今日”与“万卷”间切换，采用 **Crossfade (淡入淡出)**，时长 `500ms`。寓意心境的静谧转换，避免生硬位移。
  - **预测性返回**：手势滑动过程中，当前页与目标页同步进行淡入淡出预览。
- **层级转场 (Deep Link Transitions)**：
  - **进入路径**：从列表进入“阅读中心”或“诗人详情”时，采用 **垂直偏移滑入 (Slide Up + Fade In)**。从底部 `1/10` 处向上平滑浮起，时长 `400ms`。
  - **退出路径**：点击返回或使用**预测性返回手势**时，页面向底部 `1/10` 处滑动并淡出。
  - **一致性保证**：预测性返回手势驱动的动画参数与点击返回按钮触发的弹出动画完全一致，确保操控体验的直觉性。
